CREATE TABLE reward (
    ...
    description     VARCHAR(500)    NULL COMMENT '奖励描述',
...
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci COMMENT='奖励商品';
...
CREATE TABLE point_log (
    ...
    type            VARCHAR(20)     NOT NULL COMMENT '类型：EARN-赚取、REDEEM-兑换、MANUAL_ADD-手动增加、MANUAL_SUB-手动扣减、FREEZE-冻结、UNFREEZE-解冻退还',
    amount          INT             NOT NULL COMMENT '变动积分数（正数为增加，负数为减少）',
    balance_after   INT             NOT NULL COMMENT '操作后余额',
    ref_id          BIGINT          NULL COMMENT '关联业务ID（任务ID/订单ID等）',
